...
def setup_platform(hass, config, add_entities, discovery_info=None):
    ...
    rest = RestData(method, resource, username, password, headers, payload, verify_ssl, timeout)
    rest.login()
    rest.update()
...
class RestData:
    ...
    def __init__(
        self, method, resource, username, password, headers, data, verify_ssl, timeout=DEFAULT_TIMEOUT
    ):
    ...
    def login(self):
        url = self._resource + "/login"
        """Log in to the REST service."""
    ...
    def update(self):
        url = self._resource + "/meters?_dc=" + str(int(dt.as_timestamp(dt.now()))) + "&accountId=" + self._accountId + "&districtName=sterco&page=1&start=0&limit=100&sort=alertSeverityRank&dir=DESC"
        if (self._cookie == None):
          return
        """Get the latest data from REST service with provided method."""
        _LOGGER.debug("Updating from %s", url)
        _LOGGER.debug("Cookie header: %s", self._headers["cookie"])
        try:
            response = self._http_session.request(
                self._method,
                url,
